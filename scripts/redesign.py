...
# Polices Geist et Geist Mono auto-hébergées (fichiers variables, licence SIL OFL) : aucune requête vers Google,
# et plus de feuille de style externe bloquant le rendu. Seuls les sous-ensembles latin sont préchargés ;
# latin-ext n'est téléchargé que si un caractère l'exige.
...
def font_face(family,file,unicode_range):
 return f"@font-face{{font-family:'{family}';font-style:normal;font-weight:100 900;font-display:swap;src:url(/assets/fonts/{file}) format('woff2');unicode-range:{unicode_range}}}"
FONTS=('<link rel="preload" href="/assets/fonts/Geist-latin.woff2" as="font" type="font/woff2" crossorigin>'
 '<link rel="preload" href="/assets/fonts/GeistMono-latin.woff2" as="font" type="font/woff2" crossorigin>'
 '<style>'+font_face('Geist','Geist-latin.woff2',LATIN)+font_face('Geist','Geist-latin-ext.woff2',LATIN_EXT)
 +font_face('Geist Mono','GeistMono-latin.woff2',LATIN)+font_face('Geist Mono','GeistMono-latin-ext.woff2',LATIN_EXT)+'</style>')

def hero_preload(body):
 # L'image la plus grande au-dessus de la ligne de flottaison est préchargée en priorité haute :
 # la photo de couverture sur une page métier, sinon le ciel en fond du hero (défini dans design.css).
 cover=re.search(r'<div class="sector-cover"><img src="([^"]+)"',body)
 src=cover.group(1) if cover else '/assets/sky.webp'
...
for s in sectors:
 ...
 hero=f'''<div class="hero-shell"><header class="hero sector-hero">{nav(p)}<div class="sector-hero-grid"><div><div class="hero-eyebrow">{s['name']}</div><h1>{s['title']}</h1><p class="lead">{s['desc']}</p><a class="btn btn-lime" href="#contact">Recevoir mon audit offert <span class="arrow-circle">↗</span></a></div><div class="sector-cover"><img src="/assets/{s['photo']}.webp" fetchpriority="high" decoding="async" alt="{SEO_SECTEURS[s['slug']][2]}" width="800" height="600"><div class="floating-signal"><span class="pulse"></span>{s['signal']}<strong>{s['value']}</strong><small>Exemple de parcours</small></div></div></div></header></div>'''
...
mentions=f'''<h2>Éditeur du site</h2><p>{BRAND}, <mark>À compléter : forme juridique, capital social, adresse du siège, numéro SIREN ou RCS, numéro de TVA</mark>.</p><p>Directeur de la publication : <mark>À compléter</mark>. Contact : <mark>À compléter : adresse email</mark>.</p><h2>Hébergement</h2><p><mark>À compléter : nom de l’hébergeur, adresse et téléphone</mark>.</p><h2>Propriété intellectuelle</h2><p>Les textes, la marque {BRAND} et la mise en page de ce site appartiennent à {BRAND}. Toute reproduction sans autorisation écrite est interdite.</p><h2>Crédits</h2><p>Les photographies d’illustration proviennent d’Unsplash et ne représentent ni l’équipe ni des clients. Les chiffres affichés dans les illustrations et les simulateurs sont fictifs. Polices Geist et Geist Mono (Vercel, licence SIL Open Font License), hébergées sur ce site.</p><h2>Données personnelles</h2><p>Le traitement des informations du formulaire est décrit dans la <a href="/confidentialite">politique de confidentialité</a>.</p>'''
cookies_off='<h2>Cookies et mesure d’audience</h2><p>Ce site n’utilise aucun cookie de mesure d’audience ni de publicité. Les polices de caractères sont hébergées sur ce site : leur chargement ne transmet rien à un tiers.</p>'
cookies_on=('<h2>Cookies et mesure d’audience</h2>'
 ...
 'Les polices de caractères sont hébergées sur ce site : leur chargement ne transmet rien à un tiers.</p>')
