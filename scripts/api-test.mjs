...
try {
 ...
 assert.match(hits[1].body.client_id, /^\d+\.\d+$/); assert.notEqual(hits[1].body.client_id, booking.metadata.ga_client_id);
 assert.equal(hits[1].body.events[0].params.consentement, 'non', 'sans consentement : identifiant à usage unique');
 assert.equal(hits[1].body.events[0].params.session_id, undefined);
 ...
 assert.equal(hits[2].body.events[0].name, 'cancel_call'); assert.equal(hits[2].body.client_id, booking.metadata.ga_client_id);
 await hook('BOOKING_CANCELLED', booking); assert.equal(hits.length, 3, 'annulation comptée une seule fois');
 const moved = {...anon, uid: uid + '-moved', rescheduleUid: anon.uid, startTime: '2030-02-01T10:00:00Z'};
 assert.equal((await hook('BOOKING_RESCHEDULED', moved)).status, 200); assert.equal(hits.length, 3, 'report sans événement GA4');
