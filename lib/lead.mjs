...
const FIELDS=['nom','entreprise','secteur','email','site','tel','website_check'];
...
export async function handleLead({method,headers,readRaw}){
 ...
 const clean={};
 for(const key of FIELDS){if(data[key]!==undefined&&typeof data[key]!=='string')return reply(400,{error:'Champs invalides.'});clean[key]=(data[key]||'').trim();if(clean[key].length>MAX_FIELD)return reply(400,{error:'Un champ est trop long.'})}
 ...
 try{await sendLead(clean);return reply(200,{ok:true})}catch(e){return reply(503,{error:e.message==='NOT_CONFIGURED'?'Le service de demande d’audit n’est pas encore disponible. Votre demande n’a pas été envoyée.':'L’envoi a échoué. Veuillez réessayer dans un instant.'})}
}
