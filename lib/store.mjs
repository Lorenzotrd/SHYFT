...
// Rendez-vous Cal.com : une ligne par réservation, identifiant stable « cal-<uid> ».
export async function getLead(id) {
 if (storeMode() === 'redis') { const raw = await redis(['HGET', KEY, id]); try { return raw ? JSON.parse(raw) : null; } catch { return null; } }
 return (await localRead())[id] || null;
}

// Écrit seulement si la ligne n'existe pas encore : un webhook reçu deux fois ne compte qu'une réservation.
export async function insertLeadOnce(record) {
 if (storeMode() === 'redis') return (await redis(['HSETNX', KEY, record.id, JSON.stringify(record)])) === 1;
 const all = await localRead();
 if (all[record.id]) return false;
 all[record.id] = record; await localWrite(all); return true;
}

export async function putLead(record) {
 if (storeMode() === 'redis') await redis(['HSET', KEY, record.id, JSON.stringify(record)]);
 else { const all = await localRead(); all[record.id] = record; await localWrite(all); }
 return record;
}
