...
- Mesure : `generate_lead` et `demande_audit` sur la page merci, une seule fois par demande (ni rechargement ni visite directe), `open_booking` à l'ouverture de la fenêtre Cal.com (avec l'emplacement du bouton), toujours après consentement. Rendez-vous : fenêtre officielle Cal.com chargée au clic (`public/assets/booking.js`, lien normal en repli), métadonnées page, emplacement, UTM et, après consentement seulement, client_id et session_id GA4 ; la réservation est comptée uniquement par le serveur (`/api/cal-webhook` → `book_call`, `cancel_call` via le Measurement Protocol), jamais par le navigateur. Après réservation, toute la page part sur `/rendez-vous/merci` : dans la fenêtre intégrée, c'est `booking.js` qui le fait à l'événement `bookingSuccessfulV2` (réservation acceptée uniquement, avec identifiant, sans paiement en attente), sur la page Cal.com directe c'est la redirection configurée dans Cal.com. La page est en noindex, hors sitemap ( paramètres retirés de l'adresse) : aucun événement GA4 n'y part, la page vue sert aux conversions Google Ads (`ADS_CONVERSION_RDV` dans `src/lib/site.ts`, comptée une fois et seulement si la réservation a été ouverte depuis le site). Une conversion arrivée avant la réponse au bandeau attend la décision. Conversion Google Ads et pixel Meta prêts : renseigner `ADS_CONVERSION` et `META_PIXEL_ID` dans `src/lib/site.ts` (vides, rien n'est envoyé ; renseignés, le bandeau mentionne la mesure publicitaire, et la politique de confidentialité doit être mise à jour).
...
Vercel importe le dépôt GitHub et détecte Astro (`vercel.json`). Les pages sont servies en statique ; `/api/lead`, `/api/admin` et les routes de Keystatic tournent en fonctions. Variables d'environnement : `CAL_WEBHOOK_SECRET` (secret du webhook Cal.com, `/api/cal-webhook`, voir `lib/cal.mjs`), `GA_API_SECRET` (secret du Measurement Protocol GA4, événements `book_call` et `cancel_call` envoyés par le serveur, voir `lib/ga.mjs`), `RESEND_API_KEY` (alerte email à chaque demande, voir `lib/notify.mjs` ; destinataire `NOTIFY_EMAIL`, par défaut team@shyftgrowth.com ; expéditeur `MAIL_FROM`, par défaut demandes@shyftgrowth.com), `LEAD_WEBHOOK_URL`, `ADMIN_PASSWORD`, `KV_REST_API_URL`, `KV_REST_API_TOKEN`, et pour Keystatic en ligne `KEYSTATIC_GITHUB_CLIENT_ID`, `KEYSTATIC_GITHUB_CLIENT_SECRET`, `KEYSTATIC_SECRET` (créées par l'assistant de Keystatic à la première visite de `/keystatic` en production).
